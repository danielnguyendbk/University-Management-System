...
-- 4. USERS
-- ============================================================
INSERT INTO users (username, password_hash, email, force_password_change, role, status)
VALUES
  ('admin01', @pwd, 'admin01@ptit.edu.vn', FALSE, 'ADMIN', 'ACTIVE'),
  ('GV001', @pwd, 'tcpeduvn+GV001@gmail.com', FALSE, 'LECTURER', 'ACTIVE'),
  ('N23KHMT001', @pwd, 'tcpeduvn+N23KHMT001@gmail.com', FALSE, 'STUDENT', 'ACTIVE')
ON DUPLICATE KEY UPDATE
...
JOIN program_courses pc
    ON pc.program_id = s.program_id
JOIN courses c
    ON c.course_id = pc.course_id
JOIN course_sections cs
    ON cs.course_id = c.course_id
LEFT JOIN lecturers l
    ON l.lecturer_id = cs.lecturer_id
LEFT JOIN vw_section_capacity v
    ON v.section_id = cs.section_id
WHERE s.student_code = 'N23KHMT001'
  AND cs.semester_id = 8
  AND cs.status = 'open'
  AND (cs.class_id = s.class_id OR cs.class_id IS NULL)
...
-- Nếu @semester_id NULL thì set tay:
-- SET @semester_id = 8;

UPDATE programs
SET total_credits = 150
WHERE program_id IN (1, 3);

UPDATE programs
SET total_credits = 120
WHERE program_id = 2;

INSERT IGNORE INTO courses (
    department_id, course_code, course_name, credits, course_type, is_active
)
SELECT @department_id, x.course_code, x.course_name, 3, 'required', TRUE
FROM (
    SELECT 'IT101' course_code, 'Nhập môn Công nghệ thông tin' course_name UNION ALL
    SELECT 'PRF101', 'Lập trình cơ bản' UNION ALL
    SELECT 'MTH101A', 'Toán rời rạc' UNION ALL
    SELECT 'ENG101', 'Tiếng Anh học thuật 1' UNION ALL
    SELECT 'PHY101', 'Vật lý đại cương' UNION ALL

    SELECT 'PRO192', 'Lập trình hướng đối tượng' UNION ALL
    SELECT 'CSD201', 'Cấu trúc dữ liệu và giải thuật' UNION ALL
    SELECT 'DBI202', 'Cơ sở dữ liệu' UNION ALL
    SELECT 'MTH102A', 'Đại số tuyến tính' UNION ALL
    SELECT 'ENG102', 'Tiếng Anh học thuật 2' UNION ALL

    SELECT 'LAB211', 'Lập trình Java nâng cao' UNION ALL
    SELECT 'WED201', 'Thiết kế Web' UNION ALL
    SELECT 'OSG202', 'Nguyên lý hệ điều hành' UNION ALL
    SELECT 'SWE201', 'Nhập môn Công nghệ phần mềm' UNION ALL
    SELECT 'COM201', 'Kỹ năng giao tiếp' UNION ALL

    SELECT 'NWC203', 'Mạng máy tính' UNION ALL
    SELECT 'JPD123', 'Tiếng Nhật chuyên ngành 1' UNION ALL
    SELECT 'SWR302', 'Yêu cầu phần mềm' UNION ALL
    SELECT 'SWT301', 'Kiểm thử phần mềm' UNION ALL
    SELECT 'PRN211', 'Lập trình C#' UNION ALL

    SELECT 'SWP391', 'Dự án phát triển phần mềm' UNION ALL
    SELECT 'ITE302', 'Trí tuệ nhân tạo' UNION ALL
    SELECT 'SEC401', 'An toàn thông tin' UNION ALL
    SELECT 'PRM392', 'Phát triển ứng dụng di động' UNION ALL
    SELECT 'MLN111', 'Triết học Mác - Lênin' UNION ALL

    SELECT 'EXE101', 'Khởi nghiệp' UNION ALL
    SELECT 'DPL302', 'Pháp luật đại cương' UNION ALL
    SELECT 'WDP301', 'Phát triển ứng dụng Web' UNION ALL
    SELECT 'DMS301', 'Hệ quản trị cơ sở dữ liệu' UNION ALL
    SELECT 'CLD301', 'Điện toán đám mây' UNION ALL

    SELECT 'OJT202', 'Thực tập doanh nghiệp' UNION ALL
    SELECT 'CAP499', 'Đồ án tốt nghiệp' UNION ALL
    SELECT 'SEM499', 'Seminar chuyên ngành' UNION ALL
    SELECT 'MLN122', 'Kinh tế chính trị Mác - Lênin' UNION ALL
    SELECT 'HCM202', 'Tư tưởng Hồ Chí Minh' UNION ALL

    SELECT 'DSA301', 'Phân tích và thiết kế giải thuật' UNION ALL
    SELECT 'DWH301', 'Kho dữ liệu và khai phá dữ liệu' UNION ALL
    SELECT 'BIG401', 'Dữ liệu lớn' UNION ALL
    SELECT 'MLB401', 'Học máy' UNION ALL
    SELECT 'IOT401', 'Internet vạn vật' UNION ALL

    SELECT 'UIX301', 'Thiết kế giao diện người dùng' UNION ALL
    SELECT 'UXR302', 'Nghiên cứu trải nghiệm người dùng' UNION ALL
    SELECT 'API301', 'Thiết kế và phát triển API' UNION ALL
    SELECT 'DEV401', 'DevOps cơ bản' UNION ALL
    SELECT 'ARC401', 'Kiến trúc phần mềm' UNION ALL

    SELECT 'BSA301', 'Phân tích nghiệp vụ' UNION ALL
    SELECT 'ERP301', 'Hệ thống hoạch định nguồn lực doanh nghiệp' UNION ALL
    SELECT 'CRM301', 'Quản trị quan hệ khách hàng' UNION ALL
    SELECT 'FIN301', 'Tài chính cho nhà quản trị' UNION ALL
    SELECT 'MGT301', 'Quản trị dự án công nghệ thông tin'
) x;



INSERT IGNORE INTO program_courses (
    program_id, course_id, recommended_semester, is_required
)
SELECT 
    p.program_id,
    c.course_id,
    CEIL(x.rn / 6.25) AS recommended_semester,
    TRUE
FROM programs p
JOIN (
    SELECT 1 rn, 'IT101' code UNION ALL
    SELECT 2, 'PRF101' UNION ALL
    SELECT 3, 'MTH101A' UNION ALL
    SELECT 4, 'ENG101' UNION ALL
    SELECT 5, 'PHY101' UNION ALL
    SELECT 6, 'PRO192' UNION ALL
    SELECT 7, 'CSD201' UNION ALL
    SELECT 8, 'DBI202' UNION ALL
    SELECT 9, 'MTH102A' UNION ALL
    SELECT 10, 'ENG102' UNION ALL
    SELECT 11, 'LAB211' UNION ALL
    SELECT 12, 'WED201' UNION ALL
    SELECT 13, 'OSG202' UNION ALL
    SELECT 14, 'SWE201' UNION ALL
    SELECT 15, 'COM201' UNION ALL
    SELECT 16, 'NWC203' UNION ALL
    SELECT 17, 'JPD123' UNION ALL
    SELECT 18, 'SWR302' UNION ALL
    SELECT 19, 'SWT301' UNION ALL
    SELECT 20, 'PRN211' UNION ALL
    SELECT 21, 'SWP391' UNION ALL
    SELECT 22, 'ITE302' UNION ALL
    SELECT 23, 'SEC401' UNION ALL
    SELECT 24, 'PRM392' UNION ALL
    SELECT 25, 'MLN111' UNION ALL
    SELECT 26, 'EXE101' UNION ALL
    SELECT 27, 'DPL302' UNION ALL
    SELECT 28, 'WDP301' UNION ALL
    SELECT 29, 'DMS301' UNION ALL
    SELECT 30, 'CLD301' UNION ALL
    SELECT 31, 'OJT202' UNION ALL
    SELECT 32, 'CAP499' UNION ALL
    SELECT 33, 'SEM499' UNION ALL
    SELECT 34, 'MLN122' UNION ALL
    SELECT 35, 'HCM202' UNION ALL
    SELECT 36, 'DSA301' UNION ALL
    SELECT 37, 'DWH301' UNION ALL
    SELECT 38, 'BIG401' UNION ALL
    SELECT 39, 'MLB401' UNION ALL
    SELECT 40, 'IOT401' UNION ALL
    SELECT 41, 'UIX301' UNION ALL
    SELECT 42, 'UXR302' UNION ALL
    SELECT 43, 'API301' UNION ALL
    SELECT 44, 'DEV401' UNION ALL
    SELECT 45, 'ARC401' UNION ALL
    SELECT 46, 'BSA301' UNION ALL
    SELECT 47, 'ERP301' UNION ALL
    SELECT 48, 'CRM301' UNION ALL
    SELECT 49, 'FIN301' UNION ALL
    SELECT 50, 'MGT301'
) x
JOIN courses c ON c.course_code = x.code
WHERE p.program_id IN (1, 3)
ON DUPLICATE KEY UPDATE
...
SELECT 
    2 AS program_id,
    c.course_id,
    CEIL(x.rn / 5) AS recommended_semester,
    TRUE
FROM (
    SELECT 1 rn, 'IT101' code UNION ALL
    SELECT 2, 'PRF101' UNION ALL
