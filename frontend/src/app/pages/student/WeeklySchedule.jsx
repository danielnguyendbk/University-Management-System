...
import { getStudentTimetable, getSemesterWeeks, getStudentCalendarBlocks, getSectionOptions, getSectionTimetable } from "../../../services/studentTimetableApi";
...
export function WeeklySchedule({ showCancelled = true }) {
    const fetchSemesters = useCallback(() => registrationApi.student.getSemesters(), []);
    const fetchWeeks = useCallback((semId) => getSemesterWeeks(semId), []);
    const fetchCalendarBlocks = useCallback((semId) => getStudentCalendarBlocks(semId), []);
    ...
    // Fetch course sections when selectedSemesterId changes
    useEffect(() => {
        ...
        async function fetchSections() {
            try {
                const data = await getSectionOptions(selectedSemesterId);
                setSections(data || []);
                ...
                console.error("Lỗi tải danh sách lớp học phần:", error);
                setSections([]);
                setSelectedSectionId(null);
            }
        }
    ...
    // 3. Fetch timetable when week or view mode or section changes
    useEffect(() => {
        async function fetchTimetable() {
            ...
            try {
                ...
                if (viewMode === "PERSONAL") {
                    raw = await getStudentTimetable(fromDate, toDate);
                } else if (viewMode === "SECTION") {
                    if (selectedSectionId) {
                        raw = await getSectionTimetable(selectedSectionId, fromDate, toDate);
                    } else {
                        raw = [];
                    }
                }
                ...
                setTimetableItems([]);
                setTimetableError("Không thể tải thời khóa biểu.");
            } finally {
                setTimetableLoading(false);
            }
        }
        fetchTimetable();
    }, [selectedWeek, weekDates, viewMode, selectedSectionId]);
    ...
    const getTimeLabelTop = (time) => {
        return getTopByTime(time);
    };
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-4">
                ...
                <div className="flex flex-col gap-2 max-w-[920px]">
                    <div className="grid grid-cols-1 md:grid-cols-[330px_445px] gap-3">
                        ...
                        >
                            ...
                            <option value="SECTION">Thời khóa biểu theo lớp học phần</option>
                        </select>
                    </div>
                            ...
                            >
                                ...
                                {sections.map(sec => (
                                    <option key={sec.sectionId} value={sec.sectionId}>
                                        {sec.sectionCode} - {sec.courseName}
                                    </option>
                                ))}
                            </select>
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-[720px_auto] gap-6 items-center">
                        <select
                            ...
                            className="h-11 w-full rounded-lg border border-gray-300 bg-white px-3 text-sm text-gray-700"
                        >
                            {Array.isArray(semesterWeeks) && semesterWeeks.map((week, index) => (
            ...
            {!loading && errorMessage && <div className="text-sm text-rose-600">{errorMessage}</div>}
            {!loading && !errorMessage && timetableItems.length === 0 && (
                <div className="text-sm text-gray-500">
                    {viewMode === "SECTION"
                        ? "Lớp học phần này chưa có lịch trong tuần đã chọn."
                ...
                <div className="overflow-x-auto">
                    <div className="relative">
                        <button
                            ...
                            className="absolute left-1 top-1/2 z-30 h-8 w-8 -translate-y-1/2 font-bold text-gray-700 hover:bg-gray-50 flex items-center justify-center"
                        >
                        ...
                        <button
                            ...
                            className="absolute right-1 top-1/2 z-30 h-8 w-8 -translate-y-1/2 font-bold text-gray-700 hover:bg-gray-50 flex items-center justify-center"
                        >
                            <span className="text-2xl">→</span>
                        </button>
                                                ...
                                                <div className="text-base leading-snug flex flex-col h-full justify-between">
                                                    <div>
                                                        ...
                                                        <div className="mt-3 space-y-1.5 text-[11px] opacity-90 font-medium">
                                                            ...
                                                            <p className="truncate" title={item.lecturerName}>
                                                                GV: <span className="font-semibold">{item.lecturerName || "N/A"}</span>
                                                            </p>
                                                            {/* 'Nghỉ' Badge */}
                                                            {isOff && (
                                                                <span className="shrink-0 rounded bg-red-100 px-1.5 py-0.5 text-[9px] font-bold text-red-700 border border-red-200/80 uppercase tracking-wide animate-pulse">
                                                                    Nghỉ
                                                                </span>
                                                            )}
                                                        </div>
